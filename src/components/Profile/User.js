...
export default function UserProfile(props) {
  const {
    profile, skills, experiences, educations, onDownloadResume,
  } = props;
  return (
    <>
      <div className="profileInfoCard">
        ...
        <Cell size={10}>
          <Grid>
            <Cell size={6}>
              <FontIcon children="mail_outline" />
              {' '}
              {profile.email}
            </Cell>
            <Cell size={6}>
              <FontIcon children="phone_forwarded" />
              {' '}
              {profile.contact_number}
            </Cell>
            <Cell size={6}>
              <FontIcon children="person_pin_circle" />
              {' '}
              {formatAddress(profile.address_description, profile.address)}
            </Cell>
          </Grid>
        </Cell>
      </div>
