...
export default async function generateRiskMapExcel(data) {
  ...
  fileSaver.saveAs(blob, `prmp_${day().format('MM/DD/YYYY')}.xlsx`);
}
...
function initHeaders(ws) {
  ...
  setHeaderCell(ws, 'RISK ASSESSMENT(Risk Register)', 'A2', { extend_column: 15, fgColor: { argb:'0070C0' } });
  ...
  setHeaderCell(ws, 'Inherent', 'H3', { extend_column: 8, fgColor: {argb: 'C00000'} });
  ...
  setHeaderCell(ws, 'Residual', 'V3', { extend_column: 7, fgColor: { argb: '00B050' } });
...
function setHeaderCell(ws, value, start, options = {}) {
  const { fgColor = { argb: 'D9D9D9' }, ...restOptions } = options
  const font = {
    size: 12,
    bold: true
  }
...
function setValueAndMerge(ws, value, start, options = {}) {
  const { extend_column, extend_row, fgColor, font } = options
  let end
  if (extend_row) {
